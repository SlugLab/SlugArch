...
fn arb_msg() -> impl Strategy<Value = CxlMsg> {
    prop_oneof![
        ...
        ], any::<u64>()).prop_map(|(tag, opcode, addr)| CxlMsg::M2SReq { tag, opcode, addr }),

        (any::<u16>(), prop_oneof![
            Just(M2SRwDOp::MemWr),
            Just(M2SRwDOp::MemWrPtl),
            Just(M2SRwDOp::MemClnEvct),
        ], any::<u64>(), arb_data()).prop_map(|(tag, opcode, addr, data)|
            CxlMsg::M2SRwD { tag, opcode, addr, data }),

        (any::<u16>(), prop_oneof![
            Just(S2MDRSOp::MemData),
            Just(S2MDRSOp::MemDataNxm),
        ], arb_data()).prop_map(|(tag, opcode, data)| CxlMsg::S2MDRS { tag, opcode, data }),
