...
module slugcxl_endpoint #(
  ...
  // Load-only path: cmd_valid for one cycle, then emit Cmp.
  localparam [3:0] S_DRIVE_CMD_NO_DONE = 4'd9;
  reg [3:0]  state;
  reg [15:0] pend_tag;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state)
        S_IDLE: begin
          ...
          if (flit_in_valid && flit_in_ready) begin
            ...
            if (addr_matches_dispatch && in_class == 4'h2) begin
              ...
              // Pure loads (token_in[2]=1 but token_in[20]=0 and token_in[21]=0)
              // don't assert out_valid in the 16x16 systolic. Skip await-done
              // and emit Cmp after one drive cycle. Other RwDs (compute)
              // await done_valid.
              if (in_token_rwd[2] && !in_token_rwd[20] && !in_token_rwd[21]) begin
                state <= S_DRIVE_CMD_NO_DONE;
              end else begin
                state <= S_DRIVE_CMD;
              end
        ...
        S_DRIVE_CMD_NO_DONE: begin
          cmd_valid <= 1'b1;
          if (cmd_ready) begin
            cmd_valid <= 1'b0;
            // Skip await-done — loads don't produce out_valid.
        ...
        S_AWAIT_DONE: begin
          ...
          if (done_valid) begin
            if (pend_is_read) begin
              // The 16x16 wrapper packs read_data at token_out[23:0]
              // (u_read_data is 24 bits; token_out = 232 zeros above it).
              pend_read_data <= token_out[23:0];
