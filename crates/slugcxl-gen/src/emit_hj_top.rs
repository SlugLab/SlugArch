...
module slugcxl_4x4_hj_top #(
  ...
  parameter integer MODEL_OBSERVE_ONLY = 0
) (
  input  wire                     clk,
  input  wire                     rst_n,
  ...
  // MODEL_OBSERVE_ONLY is for a Verilator/debug driver that submits one event
  // at a time. It prevents endpoint responses from re-entering the observer.
  // The synthesizable/default value remains the connected endpoint path.
  generate
    if (MODEL_OBSERVE_ONLY != 0) begin : g_observe_only
      assign w_ep_flit_ready = 1'b1;
      assign w_ep_resp_valid = 1'b0;
      assign w_ep_resp_data = '0;
    end else begin : g_endpoint
      slugcxl_4x4_top #(
        .FLIT_BITS(FLIT_BITS),
