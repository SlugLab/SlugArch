module sovryn_pan_stem_systolic_array_16x16(
  input wire clk,
  input wire rst_n,
  ...
  integer k;
  integer idx;
  (* nomem2reg *) reg [7:0] a_mem [0:255];
  (* nomem2reg *) reg [7:0] b_mem [0:255];
  (* nomem2reg *) reg [23:0] c_mem [0:255];
  reg [31:0] sum_acc;
        ...
        for (row = 0; row < 16; row = row + 1) begin
          for (col = 0; col < 16; col = col + 1) begin
            ...
            for (k = 0; k < 16; k = k + 1) begin
              sum_acc = sum_acc + ({24'd0, a_mem[(row * 16) + k]} * {24'd0, b_mem[(k * 16) + col]});
            end
            c_mem[(row * 16) + col] = sum_acc[23:0];
          end
        end
        out_valid <= 1'b1;
